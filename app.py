"""
WaferAI v2 — Multi-Image Batch Support
Run:  pip install flask flask-cors torch torchvision opencv-python-headless Pillow numpy matplotlib
      python waferai_v2.py
...
MODEL_PATH_FP32 = os.environ.get("MODEL_PATH",     "best_model.pth")
MODEL_PATH_FP16 = os.environ.get("MODEL_PATH_FP16", "best_model_fp16.pth")
NUM_CLASSES = 8
IMG_SIZE    = 224
...
def load_model(path, use_fp16=False):
    """Load MobileNetV3-Small. If use_fp16=True, cast to half precision."""
    m = models.mobilenet_v3_small(weights=None)
    m.classifier[3] = nn.Linear(m.classifier[3].in_features, NUM_CLASSES)
    if os.path.exists(path):
        state = torch.load(path, map_location=DEVICE)
        # Accept both raw state-dicts and fp16 state-dicts saved as half tensors
        m.load_state_dict({k: v.float() for k, v in state.items()}, strict=False)
        tag = "FP16 (quantised)" if use_fp16 else "FP32 (full)"
        print(f"✅  {tag} model loaded  →  {path}")
    else:
        print(f"⚠️   {path} not found – {'FP16' if use_fp16 else 'FP32'} running on random weights")
    m.to(DEVICE)
    if use_fp16 and DEVICE.type == "cuda":
        m = m.half()   # half() only gives a real speed-up on CUDA; on CPU keep float
    m.eval()
    return m

# FP32 model  →  used exclusively for GradCAM (needs full-precision gradients)
model_fp32 = load_model(MODEL_PATH_FP32, use_fp16=False)

# FP16 model  →  used for fast classification
model_fp16 = load_model(MODEL_PATH_FP16, use_fp16=True)

# Convenience alias so the rest of the code can still reference `model` for GradCAM
model = model_fp32
...
def make_wafer_map(cls_name):
    ...
    for ry in np.arange(-1.0,1.05,dh+gap):
        for rx in np.arange(-1.0,1.05,dw+gap):
            ...
            total+=1; bad=False
            if   cls_name=="Bridge":        bad=abs(cy)<0.18 or abs(cx)<0.18          # grid lines
            elif cls_name=="Clean":         bad=False                                   # all good
            elif cls_name=="CMP Scratches": bad=abs(np.sin(np.arctan2(cy,cx)*2))<0.18 # circular scratches
            elif cls_name=="Crack":         bad=abs(cx-cy)<0.14 or abs(cx+cy)<0.14    # diagonal cracks
            elif cls_name=="LER":           bad=d>0.62 and d<0.88                      # line edge roughness ring
            elif cls_name=="Open":          bad=d<0.38                                 # center open
            elif cls_name=="Other":         bad=(int(rx*100)+int(ry*100))%5==0         # scattered random
            elif cls_name=="Vias":          bad=0.25<d<0.52 and (int(rx*100))%4==0    # via pattern
...
def run_inference(file_bytes, filename):
    ...
    # ── STAGE 1 : Fast classification via FP16 model ──────────────────────
    with torch.no_grad():
        t_in   = t.half() if (DEVICE.type == "cuda" and next(model_fp16.parameters()).dtype == torch.float16) else t
        out16  = model_fp16(t_in).float()          # cast back to float for softmax
        probs16 = torch.softmax(out16, 1)[0].cpu().numpy()
    idx_fp16  = int(probs16.argmax())
    pred      = CLASS_NAMES[idx_fp16]
    conf      = round(float(probs16[idx_fp16]) * 100, 2)
    scores    = {CLASS_NAMES[i]: round(float(probs16[i]) * 100, 2) for i in range(NUM_CLASSES)}

    # ── STAGE 2 : GradCAM explanation via FP32 model ──────────────────────
    t32 = t.clone().requires_grad_(True)
    cam, idx_fp32, out32 = gradcam.run(t32)
    # Use FP32 class index only for GradCAM heat-map; trust FP16 for reported label
    # (they usually agree; FP16 is the "official" prediction shown to the user)

    # ── Build images ──────────────────────────────────────────────────────
    orig_np = np.array(pil.resize((IMG_SIZE, IMG_SIZE)))
    ...
    fp16_loaded = os.path.exists(MODEL_PATH_FP16)
    fp32_loaded = os.path.exists(MODEL_PATH_FP32)
    ...
    return dict(
        ...
        metadata        = dict(
            filename     = filename,
            device       = str(DEVICE),
            model_fp16   = "FP16 ✓" if fp16_loaded else "FP16 (demo)",
            model_fp32   = "FP32 ✓" if fp32_loaded else "FP32 (demo)",
            pipeline     = "FP16 classify → FP32 GradCAM"
        )
    )
...
def health():
    return jsonify(
        status      = "ok",
        model       = "MobileNetV3-Small (Dual)",
        model_fp32  = str(os.path.exists(MODEL_PATH_FP32)),
        model_fp16  = str(os.path.exists(MODEL_PATH_FP16)),
        pipeline    = "FP16 classify → FP32 GradCAM",
        classes     = NUM_CLASSES,
        device      = str(DEVICE)
    )
...
def predict():
    if "image" not in request.files: return jsonify(error="No image field"),400
    f=request.files["image"]
    try: return jsonify(run_inference(f.read(),f.filename))
...
def predict_batch():
    ...
    results=[]
    for f in files:
        try: results.append(run_inference(f.read(),f.filename))
        except Exception as e: results.append(dict(success=False,metadata=dict(filename=f.filename),error=str(e)))
...
.mob-menu{display:none;flex-direction:column;gap:4px;padding:8px 16px 16px;
...
.q-name{padding:5px 7px;font-size:.6rem;color:rgba(255,255,255,.5);
  ...
  font-family:'Space Mono',monospace;background:rgba(0,0,0,.55)}
.q-status{position:absolute;top:5px;right:5px;width:20px;height:20px;
...
.sonar-ring{position:absolute;inset:0;border-radius:50%;
  border:1px solid rgba(0,212,255,.32);animation:sonar 2.2s ease-out infinite}
...
.step-dot{width:22px;height:22px;border-radius:50%;border:1px solid;
  display:flex;align-items:center;justify-content:center;flex-shrink:0;transition:all .4s}
...
/* Results */
.res-hero{padding:24px 28px;margin-bottom:16px;border:1px solid rgba(0,255,136,.18);
  box-shadow:0 0 40px rgba(0,255,136,.05)}
.conf-num{font-size:clamp(3rem,8vw,5rem);font-weight:700;font-family:'Space Mono',monospace;
...
.bc{border-radius:16px;overflow:hidden;border:1px solid var(--border);
...
.bc-file{font-size:.66rem;color:rgba(255,255,255,.32);white-space:nowrap;
...
/* History */
.hh{display:grid;padding:11px 20px;
  grid-template-columns:2.5rem 1fr 120px 90px 90px 90px;
...
.hr{display:grid;padding:12px 20px;
  grid-template-columns:2.5rem 1fr 120px 90px 90px 90px;
...
/* About */
.pipe-grid{display:grid;grid-template-columns:repeat(6,1fr);gap:16px;margin:20px 0}
.pc{display:flex;flex-direction:column;align-items:center;text-align:center}
.pi{width:54px;height:54px;border-radius:15px;background:rgba(0,212,255,.1);
  border:1px solid rgba(0,212,255,.3);display:flex;align-items:center;justify-content:center;
  margin-bottom:10px;position:relative}
.pn{position:absolute;top:-7px;right:-7px;width:18px;height:18px;border-radius:50%;
  background:var(--blue);color:#050a14;font-size:.6rem;font-weight:700;
  font-family:'Space Mono',monospace;display:flex;align-items:center;justify-content:center}
.ag{display:grid;grid-template-columns:1fr 1fr;gap:15px;margin-top:16px}
.ar{display:flex;justify-content:space-between;padding:8px 0;
  border-bottom:1px solid rgba(255,255,255,.05);font-size:.83rem}
.ar span:first-child{color:rgba(255,255,255,.4)}
.ar span:last-child{font-family:'Space Mono',monospace;color:rgba(255,255,255,.82)}
@media(max-width:960px){
  .g3{grid-template-columns:1fr}.stat-grid{grid-template-columns:repeat(3,1fr)}
  .pipe-grid{grid-template-columns:repeat(3,1fr)}.ag{grid-template-columns:1fr}
...
<div class="nav-in">
  <div class="logo" onclick="S('analyze')">
    ...
    <div><div class="logo-name">Wafer<span>AI</span></div><div class="logo-tag">PRECISION DEFECT INTELLIGENCE</div></div>
  </div>
  <div class="nav-links">
  ...
  <div class="mbadge">
    ...
    <span>FP16 classify &bull; FP32 GradCAM</span>
  </div>
...
<div class="mob-menu" id="mmenu">
  ...
  <button class="nb" onclick="S('wafer');document.getElementById('mmenu').classList.remove('open')">Wafer Map</button>
...
<div id="overlay">
  ...
  <p id="ovInfo" style="font-size:.82rem;font-family:'Space Mono',monospace;color:rgba(255,255,255,.4);margin-bottom:10px"></p>
  <h2 style="color:var(--blue);font-size:1.2rem;margin-bottom:5px">Processing...</h2>
  <p class="mono" style="color:rgba(255,255,255,.36);font-size:.76rem;margin-bottom:26px">MobileNetV3 &bull; GradCAM &bull; Wafer Map</p>
...
<div class="sec active" id="sec-analyze">
  <div class="hero fade-up">
    <div class="chip"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="#00d4ff" stroke-width="2"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="16"/><line x1="8" y1="12" x2="16" y2="12"/></svg><span>SEM DEFECT INTELLIGENCE PLATFORM</span></div>
    <h1>Real-time SEM Defect<br><span class="gt">Classification &amp; Visualization</span></h1>
    <p>Upload one or <strong style="color:var(--blue)">many SEM images at once</strong> — each gets full AI inference, GradCAM heatmap, and wafer die map.</p>
  </div>
  ...
  <div class="upload-zone" id="upZone"
    ...
    <div class="upload-icon float-anim" id="upIcon">
      <svg width="34" height="34" viewBox="0 0 24 24" fill="none" stroke="#00d4ff" stroke-width="1.5">
        <path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/>
        <polyline points="17 8 12 3 7 8"/><line x1="12" y1="3" x2="12" y2="15"/>
      </svg>
    </div>
    <p id="upLabel" style="font-size:1rem;font-weight:700;margin-bottom:6px">Drop Images Here or Click to Upload</p>
    <p id="upSub" style="font-size:.83rem;color:rgba(255,255,255,.35);margin-bottom:12px">Hold <strong style="color:var(--blue)">Ctrl/Cmd</strong> to select multiple files &bull; PNG JPG BMP TIF</p>
    <div class="pills">
    ...
    <div class="g3" style="margin-bottom:16px">
      <div class="card fade-up">
        <div class="ch"><span class="cl">ORIGINAL IMAGE</span>
          <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="rgba(255,255,255,.3)" stroke-width="2"><rect x="3" y="3" width="18" height="18" rx="2"/><circle cx="8.5" cy="8.5" r="1.5"/><polyline points="21 15 16 10 5 21"/></svg>
        </div>
        ...
        <div class="meta-rows">
          ...
          <div class="mr"><span>Classify</span><span id="mFp16" style="color:var(--amber)">FP16</span></div>
          <div class="mr"><span>GradCAM</span><span id="mFp32" style="color:var(--blue)">FP32</span></div>
          <div class="mr"><span>Pipeline</span><span id="mPipeline" style="color:var(--green);font-size:.7rem">—</span></div>
        </div>
      </div>
      <div class="card fade-up" style="animation-delay:.1s">
        <div class="ch"><span class="cl">GRADCAM THERMAL</span>
          <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="#00d4ff" stroke-width="2"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/></svg>
        </div>
      ...
      <div class="card fade-up" style="animation-delay:.2s">
        <div class="ch"><span class="cl">CLASS PROBABILITIES</span>
          <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="#00ff88" stroke-width="2"><line x1="18" y1="20" x2="18" y2="10"/><line x1="12" y1="20" x2="12" y2="4"/><line x1="6" y1="20" x2="6" y2="14"/></svg>
        </div>
...
<div class="sec" id="sec-about">
  <div style="text-align:center;padding:42px 0 26px;max-width:560px;margin:auto">
    ...
    <p style="color:rgba(255,255,255,.38);margin-top:14px;font-size:.9rem;line-height:1.7">MobileNetV3-Small fine-tuned on WM-811K SEM wafer maps. 8 defect classes. FP16 quantized to 3.2MB. GradCAM per inference. Full batch processing.</p>
  </div>
  ...
  <div class="ag">
    <div class="card" style="padding:20px">
      ...
      <div class="ar"><span>Quantization</span><span>FP32 → FP16</span></div>
...
<footer>
  <div class="fi">
    ...
    <div style="display:flex;align-items:center;gap:8px">
      ...
      <span class="mono" style="font-size:.68rem;color:rgba(255,255,255,.26)">Flask + MobileNetV3 + GradCAM &bull; Multi-Image Batch &bull; Hackathon 2025</span>
    </div>
  </div>
</footer>
</div>
...
async function runAll(){
  ...
  if(total===1){
    ...
    }catch(e){clearInterval(iv);hideOv();setQS(0,'error');showErr(e.message==='Failed to fetch'?'Cannot reach backend. Run: python waferai_v2.py':e.message);running=false;updateBtn();}
  } else {
    ...
    for(let i=0;i<Q.length;i++){
      ...
      try{
        ...
        const data=await res.json(); BR.push(data); setQS(i,'done'); addHist(data);
...
function renderSingle(d){
  ...
  document.getElementById('mFp16').textContent=d.metadata.model_fp16||'FP16';
...
if __name__ == "__main__":
    ...
    print("\\n" + "="*60)
    print("  WaferAI v2 — Dual-Model Pipeline")
    print("="*60)
    print(f"  Open:       http://localhost:{port}")
    print(f"  FP32 model: {'✅ found' if os.path.exists(MODEL_PATH_FP32) else '⚠️  not found (demo)'}")
    print(f"  FP16 model: {'✅ found' if os.path.exists(MODEL_PATH_FP16) else '⚠️  not found (demo)'}")
    print(f"  Pipeline:   FP16 classify → FP32 GradCAM")
