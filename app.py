...
def run_inference(file_bytes, filename):
    ...
    t=infer_tf(pil).unsqueeze(0).to(DEVICE); t.requires_grad_(True)
    cam,idx,out=gradcam.run(t)
    probs=torch.softmax(out,1)[0].detach().cpu().numpy()
    scores={CLASS_NAMES[i]:round(float(probs[i])*100,2) for i in range(NUM_CLASSES)}
    pred=CLASS_NAMES[idx]; conf=round(float(probs[idx])*100,2)
    ...
    return dict(success=True,predicted_class=pred,confidence=conf,scores=scores,
                original_image=orig_b64,gradcam_image=grad_b64,
                wafer_map=make_wafer_map(pred),
                metadata=dict(filename=filename,device=str(DEVICE)))

@app.route("/health")
def health():
    return jsonify(status="ok",model="MobileNetV3-Small",classes=NUM_CLASSES,device=str(DEVICE))
...
<div class="nav-in">
  ...
  <div class="mbadge">
    ...
    <span>Model v2.1 &bull; 98.0% &bull; 3.2MB</span>
  </div>
...
if __name__ == "__main__":
    ...
    print("\\n" + "="*60)
    print("  WaferAI v2 — Multi-Image Batch Support")
    print("="*60)
